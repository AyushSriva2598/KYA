...
async function callGemini(contents) {
  ...
  const response = await ai.models.generateContent({
    ...
    contents,
    config: {
      systemInstruction: `You are an autonomous web research agent.
...
export async function runAgentWorkflow({ objective, startUrl }) {
  ...
  while (iterations < maxIterations) {
    ...
    let step;
    try {
      step = await callGemini(contents);
    } catch (err) {
  ...
  if (!finalResult) {
    ...
    try {
      const fallback = await callGemini(contents);
